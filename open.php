...
function connect($thedb){
	$conn=odbc_connect($thedb,"" ,"");
	return $conn;
}

function connected($theconn){
	if($theconn){
		echo 'Made connection to the local database.</br>';
		return 1;
	}
	else{
		echo 'No connection could be made. Check that you have the name the system uses for the server.</br>
				also check that the database is not open in access - this can cause conflicts.';
		return 0;
	}
}
//include "index.php";
